...
def write_file_jq(df, path):
    ...
    for i in range(parts):
        ...
        pickle.dump(df.iloc[part * i:part * (i + 1)], open(part_path, 'wb'))
    last = df.iloc[part * parts:]
    if len(last) > 0:
        pickle.dump(last, open(path + '_part{}.pkl.gz'.format(parts), 'wb'))
...
def read_file_jq(path):
    ...
    while (True):
        part_path = path + '_part{}.pkl.gz'.format(i)
        if os.path.exists(part_path):
            arr.append(pickle.load(open(open(part_path, 'rb'))))
            i = i + 1
        else:
            break
    if len(arr) == 0:

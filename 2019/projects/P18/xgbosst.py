...
print(1)
imputation_prepare('raw_data.txt', 'pre_training.txt')
print(2)
imputation_with_spline('pre_training.txt', 'com_training.txt')
#df=pd.read_csv('com_training.txt')
#vis_imputation(df)

create_feature('com_training.txt', 'training.txt', lagging=lagging)
# add_links_lagging('data/training.txt', 'data/training1.txt')

df = pd.read_csv('training.txt', delimiter=';', parse_dates=['time_interval_begin'], dtype={'link_ID': object})

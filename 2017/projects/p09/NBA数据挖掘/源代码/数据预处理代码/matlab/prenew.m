clear;clc;
[num,player,time,hit_rate,fr_throw,reb,ast,ste,blo,tov,ppg,win,lose] = textread('result.txt','%s%s%s%s%s%s%s%s%s%s%s%s%s','delimiter', ',');
DataStr = [num,player,time,reb,ast,ste,blo,tov,ppg,win,lose];
% removing head
Data = [time,reb,ast,ste,blo,tov,ppg,win,lose];
Data(1,:) = [];
...
% writing head
fprintf(fid, '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,', DataStr{1,:});
fprintf(fid, '%s,\n', 'win_rate');
for ir = 1:ro
    % rewriting num
    fprintf(fid, '%d,', ir);

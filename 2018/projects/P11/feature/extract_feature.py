...
def Item_category_feature(data,end_time,beforeoneday):
	...
	Item_category_feature = pd.merge(item_count,beforeonedayitem_count,how='left',right_index=True,left_index=True)
	Item_category_feature = pd.merge(Item_category_feature,countAverage,how='left',right_index=True,left_index=True)
	Item_category_feature = pd.merge(Item_category_feature,buyRate,how='left',right_index=True,left_index=True)
	Item_category_feature = pd.merge(Item_category_feature,item_count_before5,how='left',right_index=True,left_index=True)
	Item_category_feature = pd.merge(Item_category_feature,item_count_before_3,how='left',right_index=True,left_index=True)
	Item_category_feature = pd.merge(Item_category_feature,item_count_before_2,how='left',right_index=True,left_index=True)
	Item_category_feature.fillna(0,inplace=True)
	return Item_category_feature

def Item_id_feature(data,end_time,beforeoneday):   
	...
	buyRate_2 = buyRate_2.replace([np.inf, -np.inf], 0)
	buyRate_3 = buyRate_3.replace([np.inf, -np.inf], 0)
	Item_id_feature = pd.merge(item_count,beforeonedayitem_count,how='left',right_index=True,left_index=True)
	Item_id_feature = pd.merge(Item_id_feature,countAverage,how='left',right_index=True,left_index=True)
	Item_id_feature = pd.merge(Item_id_feature,buyRate,how='left',right_index=True,left_index=True)
	Item_id_feature = pd.merge(Item_id_feature,item_count_unq,how='left',right_index=True,left_index=True)
	Item_id_feature = pd.merge(Item_id_feature,item_count_before5,how='left',right_index=True,left_index=True)
	Item_id_feature = pd.merge(Item_id_feature,item_count_before_3,how='left',right_index=True,left_index=True)
	Item_id_feature = pd.merge(Item_id_feature,item_count_before_2,how='left',right_index=True,left_index=True)
	Item_id_feature.fillna(0,inplace=True)
	return Item_id_feature

def User_id_feature(data,end_time,beforeoneday):   
	...
	User_id_feature = pd.merge(user_count,beforeonedayuser_count,how='left',right_index=True,left_index=True)
	User_id_feature = pd.merge(User_id_feature,countAverage,how='left',right_index=True,left_index=True)
	User_id_feature = pd.merge(User_id_feature,buyRate,how='left',right_index=True,left_index=True)
	User_id_feature = pd.merge(User_id_feature,user_count_before5,how='left',right_index=True,left_index=True)
	User_id_feature = pd.merge(User_id_feature,user_count_before_3,how='left',right_index=True,left_index=True)
	User_id_feature = pd.merge(User_id_feature,user_count_before_2,how='left',right_index=True,left_index=True)
	User_id_feature = pd.merge(User_id_feature,long_online,how='left',right_index=True,left_index=True)
	User_id_feature.fillna(0,inplace=True)
	return User_id_feature

def User_item_feature(data,end_time,beforeoneday):   
	...
	User_item_feature = pd.merge(user_item_count,beforeonedayuser_item_count,how='left',right_index=True,left_index=True)
	User_item_feature = pd.merge(User_item_feature,max_touchtime,how='left',right_index=True,left_index=True)
	User_item_feature = pd.merge(User_item_feature,max_touchtype,how='left',right_index=True,left_index=True)

	User_item_feature = pd.merge(User_item_feature,user_item_count_5,how='left',right_index=True,left_index=True)
	User_item_feature = pd.merge(User_item_feature,user_item_count_3,how='left',right_index=True,left_index=True)
	User_item_feature = pd.merge(User_item_feature,user_item_count_2,how='left',right_index=True,left_index=True)
	User_item_feature.fillna(0,inplace=True)
	return User_item_feature

def User_cate_feature(data,end_time,beforeoneday):   
	...
	User_cate_feature = pd.merge(user_item_count,beforeonedayuser_item_count,how='left',right_index=True,left_index=True)
	User_cate_feature = pd.merge(User_cate_feature,max_touchtime,how='left',right_index=True,left_index=True)
	User_cate_feature = pd.merge(User_cate_feature,max_touchtype,how='left',right_index=True,left_index=True)
	User_cate_feature = pd.merge(User_cate_feature,user_cate_count_5,how='left',right_index=True,left_index=True)
	User_cate_feature = pd.merge(User_cate_feature,user_cate_count_3,how='left',right_index=True,left_index=True)
	User_cate_feature = pd.merge(User_cate_feature,user_cate_count_2,how='left',right_index=True,left_index=True)
	User_cate_feature.fillna(0,inplace=True)
...
if __name__ == '__main__':
	...
	for i in range(15):
		...
		activeday = user_activeday(train_user_window1)
		
		a = User_id_feature(train_user_window1, LabelDay,beforeoneday)
		a = a.reset_index()

		b = Item_id_feature(train_user_window1, LabelDay,beforeoneday)
		b = b.reset_index()
		
		c = Item_category_feature(train_user_window1, LabelDay,beforeoneday)
		c = c.reset_index()
		
		d = User_cate_feature(train_user_window1, LabelDay,beforeoneday)
		d = d.reset_index()
		
		e = User_item_feature(train_user_window1, LabelDay,beforeoneday)
		e = e.reset_index()
		...
		x = pd.merge(x,activeday,left_on = ['user_id'],right_index=True,how = 'left' )
		x = x.fillna(0)
	...
	train_set.to_csv('../Output/train_set.csv',index=None)
	...
	test = pd.merge(test,activeday,left_on = ['user_id'],right_index=True,how = 'left' )
	test = test.fillna(0)
#	test.to_csv('../Output/test_set.csv',index=None)
...
#	# 保存到文件
#	predict1.to_csv("../Output/10_30_2/650_1B80minchildweight1.8.csv", index=False)
	
	predict2 = predicted.iloc[:700, [0, 1]]
	# 保存到文件
	predict2.to_csv("../Output/Result.csv", index=False)
	
#	predict3 = predicted.iloc[:750, [0, 1]]
#	# 保存到文件
#	predict3.to_csv("../Output/10_30_2/750_1B80minchildweight1.8.csv", index=False)
#	sys.exit()
#	evaluate(predicted)
